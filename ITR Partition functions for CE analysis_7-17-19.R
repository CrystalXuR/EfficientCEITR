...
Reg.mu.Exp = function(ST,A,Xs,Ms,event,data){
  ...
  fitexpT = survreg(Surv(time = ST, event = event) ~ Xs+A*Ms, dist="exponential",data=data)
  mus.ExpT = matrix(NA,N,LA)
  for(j in 1:LA){
    newdat = data.frame(Xs,A=rep(sort(unique(A))[j],N),as.matrix(Ms))
    mus.ExpT[,j] = predict(fitexpT,newdata=newdat, type = "response")
  }
  
  outExpT = list(mus.ExpT, fitexpT);names(outExpT) = c("mus.ExpT","ExpModel");outExpT
}
...
Reg.mu.gam = function(CC,A,Xs,Ms,data){
  ...
  LA = length(unique(A))    
   if(LA<2) stop("Treatment levels are less than 2")
    ...
    Coefini = coef(glm(CC ~ A, family = "Gamma"(link="identity"),data = data))
    if(dim(Ms)[2]==3){
      RegGamma = glm(CC ~ Xs+A*Ms, family = "Gamma"(link="identity"),data = data,start=c(Coefini[1],rep(0,4),Coefini[2],rep(0,6)))
    }else if(dim(Ms)[2]==2){
      RegGamma = glm(CC ~ Xs+A*Ms, family = "Gamma"(link="identity"),data = data,start=c(Coefini[1],rep(0,4),Coefini[2],rep(0,4)))
    }else{
      RegGamma = glm(CC ~ Xs+A*Ms, family = "Gamma"(link="identity"),data = data,start=c(Coefini[1],rep(0,4),Coefini[2],rep(0,2)))
    }
