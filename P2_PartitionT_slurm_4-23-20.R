...
source("./PartitionSimFunctionsT.R")
...
gammaC <- as.numeric(params[case.id,3])*2
...
for (i in 1:iter){
  ...
  cen.wm <- glm(complete ~ x3+x4+x5+A*x1+A*x2, family = "binomial", data = ObsData)
  ObsData$cen.w <- predict(cen.wm, type = "response")
  
  # Estimated restricted survival time
...
names(CCR)  <- c("iter","costModel","classTools","C_RegNaive_NP","C_AIPW_NP","C_IPW_P","C_AIPW_P")
names(EMO)  <- c("iter","costModel","classTools","Truth","E_RegNaive_NP","E_AIPW_NP","E_IPW_P","E_AIPW_P")
output.long <- do.call("rbind", out)

## saving results
write.csv(CCR,file=paste("./ResultsT/HTE_",gammaF[1],"_",gammaF[2],"WTP_",lambda,"cenrate_",cup,"ct",ct,"CCR_F3a.csv",sep=""),row.names = F)
write.csv(EMO,file=paste("./ResultsT/HTE_",gammaF[1],"_",gammaF[2],"WTP_",lambda,"cenrate_",cup,"ct",ct,"EMO_F3a.csv",sep=""),row.names = F)
write.csv(output.long,file=paste("./ResultsT/HTE_",gammaF[1],"_",gammaF[2],"WTP_",lambda,"cenrate_",cup,"ct",ct,"output_F3a.csv",sep=""),row.names = F)
