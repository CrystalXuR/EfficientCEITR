...
source("./ITR Partition functions for CE simulation_7-17-19_F3T.R")
...
for (z in 1:iter){
  ...
  Xs=select(ObsData,mainvars);Ms=select(ObsData,intvars)
  ...
  #  2   3   4 
  # 164 685 151 
  lastdat2 = select(lastdat,c("subjectid","month","time"))
  ...
  Xss=select(ObsDat3,mainvars);Mss=select(ObsDat3,c(intvars,"time"))
  for (CM in costmodel){
    if(CM=="gamma"){
...
## saving results
write.csv(CCR,file=paste("./Results_slurm_F3a/HTE_",gammaF[1],"_",gammaF[2],"WTP_",lambda,"cenrate_",cup,"CCR_F3.csv",sep=""),row.names = F)
write.csv(EMO,file=paste("./Results_slurm_F3a/HTE_",gammaF[1],"_",gammaF[2],"WTP_",lambda,"cenrate_",cup,"EMO_F3.csv",sep=""),row.names = F)
write.csv(output.long,file=paste("./Results_slurm_F3a/HTE_",gammaF[1],"_",gammaF[2],"WTP_",lambda,"cenrate_",cup,"output_F3.csv",sep=""),row.names = F)
